...
/**
 * Stores objects based on their hash. No check for collisions is made. If an object is stored for a given hash and a
 * second object is added that has the same hash, the second one is discard. All objects are stored as
...
public class HashStorage<T> {
	...
	/**
	 * Returns an object with the given hash, if one was stored and it has not been garbage collected yet. Returns null
	 * otherwise.
	 * 
	 * @param hash
	 *            the hash to look up
	...
	/**
	 * Adds an object to the store. If there already is an object with the same hash, that object is returned. Otherwise
	 * the object given as a parameter is stored and returned again. <code>null</code> can be stored if the
	 * {@link HashAlgorithm} given to the {@link #HashStorage(HashAlgorithm)} supports hashing it.
	 * 
	 * @param data
	 *            the object to add
	...
	/**
	 * Returns the algorithm used by this HashStorage.
	 * 
	 * @return the algorithm used by this HashStorage.
	 */
