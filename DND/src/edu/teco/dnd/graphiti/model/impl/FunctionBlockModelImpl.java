	...
	public FunctionBlock createBlock(final ClassLoader cl) throws InvalidFunctionBlockException {
		...
		for (OutputModel outputModel : getOutputs()) {
			...
			for (InputModel input : outputModel.getInputs()) {
				try {
					output.addConnection(new RemoteConnectionTarget(outputModel.getName(), input
							.getFunctionBlock().getID(), input.getName(), (Class<? extends Serializable>) cl
							.loadClass(input.getType())));
				} catch (ClassNotFoundException e) {
					throw new InvalidFunctionBlockException("Could not get type of remote input", e);
				}
			}
		}
