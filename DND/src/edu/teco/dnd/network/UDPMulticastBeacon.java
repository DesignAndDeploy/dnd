...
public class UDPMulticastBeacon {
	...
	/**
	 * Stores all channels that are used for sending and receiving multicasts. Maps from interface and multicast address
	 * to channel.
	 */
	 ...
	 * 
	 * @param inf
	 *            the interface to send with. Must not be null.
	 * @param address
	 *            the multicast address to use for sending. Must not be null, must be resolved and must be a multicast
	 *            address.
	 */
	...
	/**
	 * Adds an address to the list of multicast addresses that are used to send multicast packages. The address will be
	 * used with all interfaces. The first package will be send as soon as the next timer update is done.
	 * 
	 * @param address
	 *            the multicast address to use for sending. Must not be null, must be resolved and must be a multicast
	 *            address.
	 ...
	 * 
	 * @param inf
	 *            the interface from which the address should be removed
	 * @param address
	 *            the address that should be removed
	 */
	...
	private class ShutdownFuture extends DefaultFutureNotifier<Void> {
		private void check() {
			channelLock.readLock().lock();
			try {
				for (final Channel channel : channels.values()) {
